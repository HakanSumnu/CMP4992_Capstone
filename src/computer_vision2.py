...
camera = cv2.VideoCapture("data/videos/green_ball_with_green_chairs.mp4")
...
while True:
    ...
    #colorMask = cv2.inRange(frameHSV, np.array([45, 60, 0]), np.array([75, 170, 255]))
    ...
    #cv2.dilate(colorMaskedFrame, kernel=np.ones((3, 3)), dst=colorMaskedFrame, iterations=15)
    #cv2.erode(colorMaskedFrame, kernel=np.ones((3, 3)), dst=colorMaskedFrame, iterations=15)
