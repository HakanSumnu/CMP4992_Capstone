...
RATIO_OF_WIDTH_AND_PERPENDICULAR_DISTANCE = 32 / 43
RATIO_OF_HEIGHT_AND_PERPENDICULAR_DISTANCE = 32 / 43 * (3 / 4)
...
#RATIO_OF_AREA_OF_BALL_AT_PHOTO_AT_CERTAIN_DISTANCE = math.pi * 106 * 106 / (960 * 540) #In photo taken from 30cm distance and with phone.
RATIO_OF_AREA_OF_BALL_AT_PHOTO_AT_CERTAIN_DISTANCE = (math.pi * 78 * 78) / (640 * 480) #In photo taken from 50cm distance with computer cam.
...
def detectBall(frame):
    ...
    lowerYellow = np.array([25, 100, 100])
    ...
    if contours:
        ...
        if area > 100:
            ...
            if circularity > 0.7:
                ...
                #print(f"{locationVector}, {horizontalAngle / math.pi * 180}, {verticalAngle / math.pi * 180}, {distance}")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if len(locations) > 20:
            locations.pop(0)
            coefficientsOfLine, boundries = findPath(locations=locations)
            sys.stdout.write("\033[K")  # Clear the entire line
            print(f"Location: {location}, coefficients of line: {coefficientsOfLine}, boundries: {boundries}")
